...
class AccountLedger extends Page implements HasForms
{
    ...
    protected ?string $heading = 'Account Ledger';
            ...
            ->components([
                Grid::make(3)->schema([
                    Select::make('account_id')
                        ->label('Account')
                        ->options(Transaction::getJournalAccountOptions())
                        ->searchable()
                        ->required()
                        ->live(),
    ...
    public function getReportDataProperty(): array
    {
        ...
        $entries = JournalEntry::with(['transaction', 'transaction.voucherable'])
            ->where(config('accounting.table_prefix', 'acc_') . 'journal_entries.account_id', $accountId)
        ...
        return [
            'showReport' => true,
            'title' => $account->name . ($account->code ? ' (' . $account->code . ')' : '') . ' Ledger',
